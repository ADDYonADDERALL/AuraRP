...
function setIdentity(identifier, data, callback)
	MySQL.Async.execute('UPDATE `users` SET `firstname` = @firstname, `lastname` = @lastname, `dateofbirth` = @dateofbirth, `sex` = @sex, `height` = @height, `tattoos` = @tattoos WHERE identifier = @identifier', {
		['@identifier']		= identifier,
	...
	MySQL.Sync.execute(
		'INSERT INTO expsystem (identifier, kurierlvl, kurierexp, rybaklvl, rybakexp, taxilvl, taxiexp, combined, fivemcock) VALUES (@identifier, @kurierlvl, @kurierexp, @rybaklvl, @rybakexp, @taxilvl, @taxiexp, @combined, @fivemcock)',
		{
			['@identifier'] 	= identifier,
			['@kurierlvl']      = 1,
			['@kurierexp'] 		= 0,
...
AddEventHandler('esx_identity:setIdentity', function(data, myIdentifiers, plate)
	...
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			TriggerClientEvent('esx_identity:identityCheck', myIdentifiers.playerid, true)
				...
				local carname
				if randomcar == 1 then
					car = GetHashKey('rancherxl')
					carname = 'rancherxl'
				elseif randomcar == 2 then
					car = GetHashKey('blista')
					carname = 'blista'
				elseif randomcar == 3 then
					car = GetHashKey('rhapsody')
					carname = 'rhapsody'
				elseif randomcar == 4 then
					car = GetHashKey('oracle')
					carname = 'oracle'
				elseif randomcar == 5 then
					car = GetHashKey('buccaneer')
					carname = 'buccaneer'
				end
				local actualcar = {
					model = car,
			...
			TriggerEvent('d3x_vehicleshop:setVehicleOwnedfirst', _source, actualcar, 0, car, carname)
		else
