...
--[[RegisterCommand("drzwi", function(source, args, raw)
    local plyCoords = GetEntityCoords(PlayerPedId())
    for location, val in pairs(Config.Teleporters2) do
    local Enter = val['Enter']
    local Exit = val['Exit']
    local dstCheckEnter, dstCheckExit = GetDistanceBetweenCoords(plyCoords, Enter['x'], Enter['y'], Enter['z'], true), GetDistanceBetweenCoords(plyCoords, Exit['x'], Exit['y'], Exit['z'], true)
    if PlayerData.job.name == 'psycholog' and PlayerData.job.grade_name == 'boss' then
        if dstCheckEnter <= 1.2 then
    if otwarte == nil then
        otwarte = true
        ESX.ShowNotification('~g~Drzwi zostały otwarte')
    elseif otwarte == true then
        otwarte = false
        ESX.ShowNotification('~r~Drzwi zostały zamknięte')
    elseif otwarte == false then
        otwarte = true
        ESX.ShowNotification('~g~Drzwi zostały otwarte')
            end
        end
...
function Teleport2(table, location)
    if not otwarte then
        ESX.ShowNotification('~r~Drzwi są zamknięte')
    elseif location == 'enter' then
        DoScreenFadeOut(100)
        Citizen.Wait(750)
        ESX.Game.Teleport(PlayerPedId(), table['Exit'])
        DoScreenFadeIn(100)
