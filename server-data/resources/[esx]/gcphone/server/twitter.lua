  ...
  getUser(username, password, function (user)
    if user == nil then
      if sourcePlayer ~= nil then
        ...
        TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, "Nieprawidłowy Login lub Hasło!")
      end
      return
    end
    MySQL.Async.insert("INSERT INTO twitter_tweets (`authorId`, `message`, `realUser`) VALUES(@authorId, @message, @realUser);", {
      ...
      MySQL.Async.fetchAll('SELECT * from twitter_tweets WHERE id = @id', {
        ...
        TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, "Wysłano Tweeta!")
        TriggerEvent('gcPhone:twitter_newTweets', tweet)
        TriggerClientEvent('gcPhone:nowyPost', -1)
      end)
    end)
  end)
end
...
function TwitterCreateAccount(username, password, avatarUrl, cb)
  if avatarUrl == '' then avatarUrl = 'https://i.imgur.com/tKRwbhU.png' end
...
AddEventHandler('gcPhone:twitter_login', function(username, password)
  print('Event: gcPhone:twitter_login')
  ...
  getUser(username, password, function (user)
    if user == nil then
      ...
      --TriggerClientEvent("sendProximityMessageTweet", -1, source, 'APP_TWITTER_NOTIF_LOGIN_SUCCESS')
      TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, "Zalogowano Pomyślnie!")
...
AddEventHandler('gcPhone:twitter_changePassword', function(username, password, newPassword)
  print('Event: gcPhone:twitter_changePassword')
  local sourcePlayer = tonumber(source)
  getUser(username, password, function (user)
    if user == nil then
      --TwitterShowError(sourcePlayer, 'Twitter Info', 'APP_TWITTER_NOTIF_NEW_PASSWORD_ERROR')
      --TriggerClientEvent("sendProximityMessageTweet", -1, source, 'APP_TWITTER_NOTIF_NEW_PASSWORD_ERROR')
      ...
      MySQL.Async.execute("UPDATE `twitter_accounts` SET `password`= @newPassword WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
        ...
        if (result == 1) then
          ...
          --TriggerClientEvent("sendProximityMessageTweet", -1, source, 'APP_TWITTER_NOTIF_NEW_PASSWORD_SUCCESS')
          TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, "Twoje hasło zostało zmienione!")
        else
          --TwitterShowError(sourcePlayer, 'Twitter Info', 'APP_TWITTER_NOTIF_NEW_PASSWORD_ERROR')
          --TriggerClientEvent("sendProximityMessageTweet", -1, source, 'APP_TWITTER_NOTIF_NEW_PASSWORD_ERROR')
          TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, "Wystąpił błąd podczas zmiany Hasła. Spróbuj ponownie za kilka sekund!")
...
AddEventHandler('gcPhone:twitter_createAccount', function(username, password, avatarUrl)
  print('Event: gcPhone:twitter_createAccount')
  local sourcePlayer = tonumber(source)
  TwitterCreateAccount(username, password, avatarUrl, function (id)
    if (id ~= 0) then
      ...
      --TriggerClientEvent("sendProximityMessageTweet", -1, source, 'APP_TWITTER_NOTIF_ACCOUNT_CREATE_SUCCESS')
      TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, "Twoje konto na Twitterze zostało utworzone!")
    else
      --TwitterShowError(sourcePlayer, 'Twitter Info', 'APP_TWITTER_NOTIF_ACCOUNT_CREATE_ERROR')
      --TriggerClientEvent("sendProximityMessageTweet", -1, source, 'APP_TWITTER_NOTIF_ACCOUNT_CREATE_ERROR')
      TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, "Nazwa użytkownika jest zajęta!")
    end
  end)
end)
...
AddEventHandler('gcPhone:twitter_getTweets', function(username, password)
  print('Event: gcPhone:twitter_getTweets')
  local sourcePlayer = tonumber(source)
...
AddEventHandler('gcPhone:twitter_getFavoriteTweets', function(username, password)
  print('Event: gcPhone:twitter_getFavoriteTweets')
  local sourcePlayer = tonumber(source)
  if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
    getUser(username, password, function (user)
      local accountId = user and user.id
...
AddEventHandler('gcPhone:twitter_postTweets', function(username, password, message)
  print('Event: gcPhone:twitter_postTweets')
  local sourcePlayer = tonumber(source)
...
AddEventHandler('gcPhone:twitter_toogleLikeTweet', function(username, password, tweetId)
  print('Event: gcPhone:twitter_toogleLikeTweet')
  local sourcePlayer = tonumber(source)
...
AddEventHandler('gcPhone:twitter_setAvatarUrl', function(username, password, avatarUrl)
  print('Event: gcPhone:twitter_setAvatarUrl')
  local sourcePlayer = tonumber(source)
  MySQL.Async.execute("UPDATE `twitter_accounts` SET `avatar_url`= @avatarUrl WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
    ...
    if (result == 1) then
      ...
      --TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, 'APP_TWITTER_NOTIF_AVATAR_SUCCESS')
      --TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, "Nazwa użytkownika jest zajęta!")
      TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, "Twóje zdjęcie zostało zmienione!")
    else
      --TwitterShowError(sourcePlayer, 'Twitter Info', 'APP_TWITTER_NOTIF_LOGIN_ERROR')
      --TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, 'Nieprawidłowy Login lub Hasło!')
      --TriggerClientEvent("sendProximityMessageMe", -1, sourcePlayer, 'Nieprawidłowy Login lub ', 'Hasło!')
      TriggerClientEvent("sendProximityMessageTweet", -1, sourcePlayer, "Nieprawidłowy Login lub Hasło!")
    end
  end)
end)
...
AddEventHandler('gcPhone:twitter_newTweets', function (tweet)
  print('Event: gcPhone:twitter_newTweets')
