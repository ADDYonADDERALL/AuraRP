...
RegisterServerEvent('esx_weashop:ladujlicke')
AddEventHandler('esx_weashop:ladujlicke', function (source)
  LoadLicenses(source)
end)
...
AddEventHandler('esx_weashop:buyItem', function(itemName, price, zone)
  ...
  local itemdata = getitemname(itemName)
  local itemnamee = itemdata.label
  ...
  if zone=="BlackWeashop" then
      if account.money >= price then
          ...
          TriggerEvent("logi:weashopblack", _source, xPlayer.identifier, xPlayer.name, itemName)
		    else
			    xPlayer.addInventoryItem(itemName, 1)
          TriggerClientEvent('esx:showNotification', _source, _U('buy') .. itemnamee)
          TriggerEvent("logi:weashopblack", _source, xPlayer.identifier, xPlayer.name, ESX.GetWeaponLabel(itemName))
		    end
		...
		if itemName == "clip" then
			xPlayer.addInventoryItem(itemName, 1)
      TriggerClientEvent('esx:showNotification', _source, _U('buy') .. "Magazynek")
      TriggerEvent("logi:weashop", _source, xPlayer.identifier, xPlayer.name, itemName)
		else
			xPlayer.addInventoryItem(itemName, 1)
      TriggerClientEvent('esx:showNotification', _source, _U('buy') .. itemnamee)
      TriggerEvent("logi:weashop", _source, xPlayer.identifier, xPlayer.name, ESX.GetWeaponLabel(itemName))
		end
...
--[[
RegisterServerEvent('esx_weashop:dajinfo')
AddEventHandler('esx_weashop:dajinfo', function()
  local _source = source
  local xPlayer  = ESX.GetPlayerFromId(source)
  ...
  if account >= price then 
    xPlayer.removeMoney(price)
	TriggerClientEvent('esx_weashop:infocorrect', _source)
  else
