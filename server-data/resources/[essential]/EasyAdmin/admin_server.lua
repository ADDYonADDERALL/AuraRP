...
Colours                = {}
Colours.green          = 56108
Colours.grey           = 8421504
Colours.red            = 16711680
Colours.orange         = 16744192
Colours.blue           = 2061822
Colours.purple         = 11750815
...
Citizen.CreateThread(function()
	...
	AddEventHandler('logsbanCheaterr', function(playerId,reason)
...
local DiscordWebHook = "https://discordapp.com/api/webhooks/738900580822679652/792zfWpDll6y4pSQjUONusPJEL8FaCJD5l7R_Hxk3-V5bmPPd0pnz3bBU7ApMSc9lNo_"
local embeds = {{["title"]=message,["type"]="rich",["color"] =color,["footer"]={["text"]= "AuraRP-EA",},}}
if message == nil or message == '' then return FALSE end
PerformHttpRequest(DiscordWebHook, function(err, text, headers) end, 'POST', json.encode({ username = name,embeds = embeds}), { ['Content-Type'] = 'application/json' })
end

RegisterServerEvent("EasyAdmin:AdminMonitor")
AddEventHandler("EasyAdmin:AdminMonitor", function(what)
  local _source = source
  local xPlayer = ESX.GetPlayerFromId(_source)
  local steamhex = GetPlayerIdentifier(_source)
  	if what == 'invisible' then
  		sendToDiscord('EASYADMIN INVISIBLE ON','ADMIN: ' .. GetPlayerName(source) .. '\nID: '.._source..'\nHEX: '..steamhex, Colours.green)
	elseif what == 'invisibleoff' then
		sendToDiscord('EASYADMIN INVISIBLE OFF','ADMIN: ' .. GetPlayerName(source) .. '\nID: '.._source..'\nHEX: '..steamhex, Colours.red)
	elseif what == 'noclipon' then
		sendToDiscord('EASYADMIN NOCLIP ON','ADMIN: ' .. GetPlayerName(source) .. '\nID: '.._source..'\nHEX: '..steamhex, Colours.green)
	elseif what == 'noclipoff' then
		sendToDiscord('EASYADMIN NOCLIP OFF','ADMIN: ' .. GetPlayerName(source) .. '\nID: '.._source..'\nHEX: '..steamhex, Colours.red)
	elseif what == 'godmodeon' then
		sendToDiscord('EASYADMIN GODMODE ON','ADMIN: ' .. GetPlayerName(source) .. '\nID: '.._source..'\nHEX: '..steamhex, Colours.green)
	elseif what == 'godmodeoff' then
		sendToDiscord('EASYADMIN GODMODE OFF','ADMIN: ' .. GetPlayerName(source) .. '\nID: '.._source..'\nHEX: '..steamhex, Colours.red)
	elseif what == 'playerblipson' then
		sendToDiscord('EASYADMIN PLAYERBLIPS ON','ADMIN: ' .. GetPlayerName(source) .. '\nID: '.._source..'\nHEX: '..steamhex, Colours.green)
	elseif what == 'playerblipsoff' then
		sendToDiscord('EASYADMIN PLAYERBLIPS OFF','ADMIN: ' .. GetPlayerName(source) .. '\nID: '.._source..'\nHEX: '..steamhex, Colours.red)
	elseif what == 'rainbowcaron' then
		sendToDiscord('EASYADMIN RAINBOWCAR ON','ADMIN: ' .. GetPlayerName(source) .. '\nID: '.._source..'\nHEX: '..steamhex, Colours.green)
	elseif what == 'rainbowcaroff' then
		sendToDiscord('EASYADMIN RAINBOWCAR OFF','ADMIN: ' .. GetPlayerName(source) .. '\nID: '.._source..'\nHEX: '..steamhex, Colours.red)
	end
end)

RegisterServerEvent("EasyAdmin:requestrevive")
AddEventHandler("EasyAdmin:requestrevive", function(guyid)
	local _source = source
  	local xPlayer = ESX.GetPlayerFromId(_source)
  	local steamhex = GetPlayerIdentifier(_source)
  	local TxPlayer = ESX.GetPlayerFromId(guyid)
  	local Tsteamhex = GetPlayerIdentifier(guyid)
	if DoesPlayerHavePermission(_source,"easyadmin.spectate") then
		TriggerClientEvent('esx_ambulancejob:revivee', tonumber(guyid))
		TriggerClientEvent('esx:showNotification', _source, '~y~ID: ~b~'..tonumber(guyid)..'~s~ dostał revive!')
		TriggerClientEvent('esx:showNotification', tonumber(guyid), '~y~ADMIN: ~b~'.. GetPlayerName(_source) ..'~s~ podał ci ręke!')
		sendToDiscord('EASYADMIN REVIVE','ADMIN: ' .. GetPlayerName(_source) .. '\nID: '.._source..'\nHEX: '..steamhex..'\nDal REV: '.. GetPlayerName(guyid) .. '\nID: '..guyid..'\nHEX: '..Tsteamhex, Colours.green)
	end
end)