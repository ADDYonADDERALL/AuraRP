...
ESX.ShowInventory = function()
  ESX.TriggerServerCallback("sandycards:getSim", function(result2)
    SimTab = result2
  end)
	...
	if ESX.PlayerData.money > 0 then
		table.insert(elements, {
		  label     = '[Gotówka] $' .. ESX.PlayerData.money,
		  description = 'gotówkę: $',
	  ...
	  for i = 1, #ESX.PlayerData.accounts, 1 do
		if ESX.PlayerData.accounts[i].money > 0 and ESX.PlayerData.accounts[i].name ~= 'bank' then
		  table.insert(elements, {
			...
			description = ESX.PlayerData.accounts[i].label:lower() .. ': $',
	  ...
	  for i = 1, #ESX.PlayerData.inventory, 1 do
		if ESX.PlayerData.inventory[i].count > 0 then
		  table.insert(elements, {
			label     = 'x' .. ESX.PlayerData.inventory[i].count .. ' ' .. ESX.PlayerData.inventory[i].label,
			description = ESX.PlayerData.inventory[i].label,
	...
	--[[
	  for i = 1, #Config.Weapons, 1 do
		...
		if HasPedGotWeapon(playerPed,  weaponHash,  false) and Config.Weapons[i].name ~= 'WEAPON_UNARMED' then
		  ...
		  table.insert(elements, {
			label     = 'x1 ' .. Config.Weapons[i].label .. ' [' .. ammo .. ']',
			description = 'x1 ' .. Config.Weapons[i].label,
			count     = 1,
			type      = 'item_weapon',
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory',
	{
		...
		if data.current.usable then
			if data.current.type == 'item_simcard' then
				TriggerEvent('sandy:idkkurwamozedziala', data.current)
			else
				...
				if data1.current.action == 'give' then
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to',
					{
						...
						if type == 'item_weapon' then
							...
							giveanim()
							TriggerServerEvent('esx:giveInventoryItemasek', GetPlayerServerId(nearbyPlayer), type, item, sourceAmmo)
							menu2.close()
							menu1.close()
							...
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
								...
								if quantity ~= nil then
									giveanim()
									TriggerServerEvent('esx:giveInventoryItemasek', GetPlayerServerId(nearbyPlayer), type, item, quantity)
							...
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
								...
								if quantity ~= nil then
									if quantity <= pedAmmo and quantity >= 0 then
										...
										TriggerServerEvent('esx:sandyammo', GetPlayerServerId(closestPlayer), item, quantity)
