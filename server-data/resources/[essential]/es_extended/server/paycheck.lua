		...
		for i=1, #xPlayers, 1 do
			...
			if salary > 0 then
				...
				elseif Config.EnableSocietyPayouts then -- possibly a society
					TriggerEvent('esx_societymordo:getSociety', xPlayer.job.name, function (society)
					...
					xPlayer.addAccountMoney('bank', salary)
					TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck'), 'Wyplata: ~y~'..salary..'$~w~ z: ~b~'..xPlayer.job.label, 'CHAR_BANK_MAZE', 9)
				end
			end
			--SECONDJOB INCLUDED	
			if salary2 > 0 then  	
				...
				elseif Config.EnableSocietyPayouts then -- possibly a society	
					TriggerEvent('esx_societymordo:getSociety', xPlayer.job2.name, function (society)	
						if society ~= nil then -- verified society	
							TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function (account)	
					...
					xPlayer.addAccountMoney('bank', salary2)	
					TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck2'), 'Wyplata: ~y~'..salary2..'$~w~ z: ~b~'..xPlayer.job2.label, 'CHAR_BANK_MAZE', 9)	
				end	
			end	
