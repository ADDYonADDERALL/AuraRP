...
Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/e', 'Zagraj animacje', {{ name="emotename", help="taniec, kamera, siedzienie lub inna ważna emotka."}})
    TriggerEvent('chat:addSuggestion', '/e', 'Zagraj animacje', {{ name="emotename", help="taniec, kamera, siedzienie lub inna ważna emotka."}})
    TriggerEvent('chat:addSuggestion', '/emote', 'PZagraj animacje', {{ name="emotename", help="taniec, kamera, siedzienie lub inna ważna emotka."}})
    if Config.SqlKeybinding then
      TriggerEvent('chat:addSuggestion', '/emotebind', 'Zbinduj animacje', {{ name="key", help="num4, num5, num6, num7. num8, num9. Numpad 4-9!"}, { name="emotename", help="taniec, kamera, siedzieć lub inna ważna emotka."}})
      TriggerEvent('chat:addSuggestion', '/emotebinds', 'Sprawdź aktualnie zbindowane animacje.')
    end
    TriggerEvent('chat:addSuggestion', '/emotemenu', 'Domyślnie otwórz menu animacji (F3).')
    TriggerEvent('chat:addSuggestion', '/emotes', 'Wyświetl dostępne animacje.')
    TriggerEvent('chat:addSuggestion', '/walk', 'Ustaw swój styl chodzenia.', {{ name="style", help="/walks po listę prawidłowych stylów"}})
    TriggerEvent('chat:addSuggestion', '/walks', 'Lista dostępnych stylów chodzenia.')
end)
...
function EmoteCancel()
  ...
  if ChosenDict == "MaleScenario" and IsInAnimation then
    ...
    IsInAnimation = false
    DebugPrint("Wymuszone wyjście ze scenariusza")
...
function EmoteChatMessage(args)
  if args == display then
    TriggerEvent("chatMessage", "^2Pomoc^0", {0,0,0}, string.format(""))
  else
    TriggerEvent("chatMessage", "^2Pomoc^0", {0,0,0}, string.format(""..args..""))
  end
end
...
function DestroyAllProps()
  ...
  PlayerHasProp = false
  DebugPrint("Zniszczone rekwizyty")
end
...
function CheckGender()
  ...
  DebugPrint("Ustaw płeć jako = ("..PlayerGender..")")
end
