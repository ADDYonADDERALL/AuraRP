...
AddEventHandler('esx_hifi:play_music', function(id, object)
    if distance(object) < Config.distance then
        ...
        Citizen.CreateThread(function()
            while lastobject == object do
                Citizen.Wait(100)
                volume = setsVolume - distance(object) * 4
                if volume < 0 then
                    volume = 0
                    Wait(100)
                end
...
function OpenhifiMenu()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'hifi', {
        ...
        local lCoords = GetEntityCoords(GetClosestObjectOfType(GetEntityCoords(PlayerPedId()), 3.0, GetHashKey('prop_boombox_01'), false, false, false))

        if data.current.value == 'get_hifi' and (GetGameTimer() - GUI.Time) > 2000 then
            menu.close()
...
Citizen.CreateThread(function()
    while true do
        ...
        if currentAction then
            if IsControlJustReleased(0, 38) and currentAction == 'music' and (GetGameTimer() - GUI.Time) > 2000 then
                OpenhifiMenu()
                GUI.Time = GetGameTimer()
            end
        else
